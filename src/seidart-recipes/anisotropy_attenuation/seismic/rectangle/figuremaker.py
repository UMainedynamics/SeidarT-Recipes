...
from seidart.routines.materials import tensor2velocities
...
direction1 = direction[ircx1,:] / np.linalg.norm(direction[ircx1,:]) # isotropic
direction2 = direction[ircx2,:] / np.linalg.norm(direction[ircx2,:]) # isotropic with attenuation
direction = array_vx2.receiver_xyz - array_vx2.source_xyz 
direction3 = direction[ircx1,:] / np.linalg.norm(direction[ircx1,:]) # anisotropic
direction4 = direction[ircx2,:] / np.linalg.norm(direction[ircx2,:]) # anisotropic with attenuation
...
# The christoffel equations are returning the incorrect values for the isotropic tensor
vpv, vph, vsv, vsh = tensor2velocities(seis.stiffness_coefficients.loc[1])
array_vx1.srcrcx_distance()
dist = array_vx1.distances[ircx2]
velocities2 = np.array([vsh, vsv, vph])
velocities3 = velocities2.copy() 
...
vx[:,0] = array_vx1.timeseries[:,ircx1]
vx[:,1] = array_vx1.timeseries[:,ircx2]
vx[:,2] = array_vx2.timeseries[:,ircx1]
vx[:,3] = array_vx2.timeseries[:,ircx2]

vy = np.zeros([seis.time_steps, 4])
vy[:,0] = array_vy1.timeseries[:,ircx1]
vy[:,1] = array_vy1.timeseries[:,ircx2]
vy[:,2] = array_vy2.timeseries[:,ircx1]
vy[:,3] = array_vy2.timeseries[:,ircx2]

vz = np.zeros([seis.time_steps, 4])
vz[:,0] = array_vz1.timeseries[:,ircx1]
vz[:,1] = array_vz1.timeseries[:,ircx2]
vz[:,2] = array_vz2.timeseries[:,ircx1]
vz[:,3] = array_vz2.timeseries[:,ircx2]
