...
zop1.co_run()
zop1.exaggeration = 0.03
zop1.sectionplot() 


depth = (zop1.receiver_xyz_all[:,2] - domain.cpml ) * domain.dz

zop1_txrx_dist = np.sqrt( 
    np.sum( 
        (zop1.source_xyz - (zop1.receiver_xyz_all-domain.cpml)*domain.dz)**2,
        axis = 1
    )
)
fig, ax = plt.subplots(1,1, figsize = (4,4) )
ax.plot(depth, zop1_txrx_dist, lw = 2, c='k')
ax.set_ylim(20,40)
