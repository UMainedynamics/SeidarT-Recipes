...
exaggeration = 0.03
rcx_depth = (zop2.receiver_xyz_all[:,2] - 65)*domain.dz
rcx_tick_label = rcx_depth[::15]
rcx_tick_locs = np.arange(0, len(rcx_depth))[::15]
timevals = np.arange(0, zop2.electromag.time_steps)
timeval_locs = timevals[::200]
timeval_labels = np.round(timeval_locs * zop2.electromag.dt / 1e-9).astype(int)

# ax0 = plt.subplot(gs[0]) 
im = ax0.imshow(
    zop2.timeseries, cmap = 'Greys', aspect = 'auto', 
    extent = [5, 80, zop2.electromag.time_steps, 0],
    origin = 'upper'
)
...
ax1.plot(rcx_depth, zop2.distances, 'k', lw=2)
